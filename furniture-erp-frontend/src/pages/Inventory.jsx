        ...
        {stockItems.map(stockData => (
          <div key={stockData.skuCode} className="glass-panel animate-fade-in" style={{ padding: '32px', display: 'flex', flexDirection: 'column', gap: '24px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid var(--border-glass)', paddingBottom: '16px' }}>
              ...
              <div>
                <span className={`badge ${stockData.quantity > 0 ? 'badge-success' : 'badge-error'}`} style={{ fontSize: '0.8rem', padding: '6px 12px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                <span style={{ color: 'var(--text-secondary)', fontSize: '0.8rem' }}>Quantity</span>
                <span style={{ fontSize: '2rem', fontWeight: 'bold' }}>{stockData.quantity}</span>
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                <span style={{ color: 'var(--text-secondary)', fontSize: '0.8rem' }}>Location</span>
                <span style={{ fontSize: '1.2rem', fontWeight: 'bold', display: 'flex', alignItems: 'center', gap: '8px' }}>
                  <Box size={18} color="var(--accent-primary)" /> {stockData.locationBin}
                </span>
              </div>
            </div>
          </div>
